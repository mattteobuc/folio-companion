...
export default function DashboardPage() {
  ...
  const ensureDefaultPortfolio = useCallback(async (userId: string) => {
    ...
    const { data: newPortfolio, error: portfolioInsertError } = await supabase
      .from("portfolios")
      .insert({ user_id: userId, name: "Portafoglio principale" })
      .select("id")
      .single<PortfolioRow>();
  ...
  const loadNews = useCallback(async () => {
    ...
    try {
      ...
      setNewsErrorMessage(error instanceof Error ? error.message : "Errore inatteso durante il caricamento notizie.");
    } finally {
  ...
  const loadMacroContext = useCallback(async (options?: { forceRefresh?: boolean }) => {
    ...
    try {
      if (!shouldForceRefresh) {
        ...
        if (rawCachedMacro) {
          ...
          if (isFresh && parsedCache.testo && parsedCache.data) {
            setMacroText(parsedCache.testo);
            setMacroDate(parsedCache.data);
            return;
          }
        }
      }
      ...
      setMacroDate(payload.data || "");
      window.localStorage.setItem(MACRO_CACHE_KEY, JSON.stringify({
        testo: payload.testo || "",
        data: payload.data || "",
        timestamp: Date.now(),
      }));
      ...
      setMacroErrorMessage(error instanceof Error ? error.message : "Errore inatteso durante il caricamento del contesto macro.");
    } finally {
  ...
  const handleAddAsset = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!Number.isFinite(parsedAmount) || parsedAmount <= 0) {
      setErrorMessage("Inserisci un importo investito valido maggiore di zero.");
      return;
    }
    ...
    <main className="min-h-screen bg-zinc-50 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100">
      <header className="border-b border-zinc-200 bg-white/90 backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/90">
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          <p className="text-lg font-semibold tracking-tight text-blue-700 dark:text-blue-400">Folio Companion</p>
          <div className="flex items-center gap-3">
            <a href="/checkin" className="hidden rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm font-medium text-zinc-700 transition hover:border-blue-300 hover:text-blue-700 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200 dark:hover:border-blue-500 dark:hover:text-blue-400 sm:inline-flex">
              Check-in settimanale
            </a>
            <span className="hidden text-sm text-zinc-600 dark:text-zinc-300 sm:inline">{userEmail || "Utente"}</span>
            <button type="button" onClick={handleLogout} disabled={isLoggingOut} className="inline-flex items-center justify-center rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm font-medium text-zinc-700 transition hover:border-blue-300 hover:text-blue-700 disabled:cursor-not-allowed disabled:opacity-60 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200 dark:hover:border-blue-500 dark:hover:text-blue-400">
          ...
          <div className="space-y-8 lg:col-span-3">
            ...
            <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
              <div className="mb-3 flex items-center justify-between gap-3">
                ...
                <button type="button" onClick={() => void loadMacroContext({ forceRefresh: true })} disabled={isMacroLoading} className="inline-flex items-center rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm font-medium text-zinc-700 transition hover:border-blue-300 hover:text-blue-700 disabled:cursor-not-allowed disabled:opacity-70 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200 dark:hover:border-blue-500 dark:hover:text-blue-400">
              ...
              {macroDate && (
                <p className="mb-3 inline-flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400">
                  ...
                  <span suppressHydrationWarning>{macroDate}</span>
                </p>
              )}
              {macroErrorMessage ? (
            ...
            <div>
              <div className="mb-5 flex items-center justify-between">
                ...
                <button type="button" onClick={() => setIsAddAssetModalOpen(true)} className="inline-flex items-center justify-center rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-blue-500">
                  Aggiungi asset
                </button>
              </div>
              {errorMessage && (
                        ...
                        assets.map((asset) => {
                          ...
                          return (
                            <tr key={asset.id} className="text-sm">
                              ...
                              <td className="px-4 py-3">
                                <button type="button" onClick={() => void handleDeleteAsset(asset.id)} disabled={deletingAssetId === asset.id} className="rounded-md px-2 py-1 text-xs font-medium text-red-600 transition hover:bg-red-50 disabled:opacity-50 dark:text-red-400 dark:hover:bg-red-950/30">
            ...
            <div className="flex h-full flex-col overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
              ...
              <div className="border-t border-zinc-200 p-4 dark:border-zinc-800">
                ...
                <form onSubmit={handleSendChatMessage} className="flex items-center gap-2">
                  <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} placeholder="Scrivi un messaggio..." className="w-full rounded-full border border-zinc-300 bg-white px-4 py-2.5 text-sm outline-none ring-blue-200 transition focus:border-blue-500 focus:ring-4 dark:border-zinc-700 dark:bg-zinc-950" />
                  <button type="submit" disabled={isSendingChat || !chatInput.trim()} className="inline-flex items-center justify-center rounded-full bg-blue-600 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-70">Invia</button>
                </form>
              </div>
            </div>
          </aside>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/50 p-4">
          <div className="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-800 dark:bg-zinc-900">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button type="button" onClick={() => { setIsAddAssetModalOpen(false); resetAssetForm(); }} className="rounded-md p-1 text-zinc-500 transition hover:bg-zinc-100 hover:text-zinc-700 dark:hover:bg-zinc-800 dark:hover:text-zinc-200" aria-label="Chiudi modal">✕</button>
            </div>
            <form onSubmit={handleAddAsset} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative" ref={symbolSearchContainerRef}>
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setShowSymbolDropdown(true);
                      if (!selectedAsset || v !== `${selectedAsset.name} (${selectedAsset.ticker})`) {
                        setSelectedAsset(null);
                      }
                      if (v.trim().length < 3) {
                        setAssetSearchResults([]);
                        ...
                        assetSearchResults.map((result) => (
                          <button
                            ...
                            onClick={() => {
                              ...
                              setAssetSearchQuery(`${result.name} (${result.ticker})`);
                              setShowSymbolDropdown(false);
                            }}
                ...
                {selectedAsset && (
                  <p className="mt-1.5 text-xs text-emerald-600 dark:text-emerald-400">
                    ✓ {selectedAsset.name} · {selectedAsset.asset_type} · {selectedAsset.market}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <input
                  type="date"
                  value={purchaseDate}
                  onChange={(e) => { setPurchaseDate(e.target.value); setHistoricalPriceError(null); }}
                  required
              ...
              {/* Errore prezzo storico */}
              {historicalPriceError && (
                <p className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-700 dark:border-amber-900/60 dark:bg-amber-950/40 dark:text-amber-300">
                  ⚠ {historicalPriceError}
                </p>
              )}
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                <button type="button" onClick={() => { setIsAddAssetModalOpen(false); resetAssetForm(); }} className="rounded-lg border border-zinc-300 px-4 py-2 text-sm font-medium text-zinc-700 transition hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-200 dark:hover:bg-zinc-800">Annulla</button>
                <button type="submit" disabled={isSaveButtonDisabled} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-70">
